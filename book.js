...
(function codeSnippets() {
  ...
  function run_rust_code(code_block) {
    ...
    //   })
    //   .join("&");
    // 编译错误 {"errors":{"File.kt":[{"interval":{"start":{"line":20,"ch":16},"end":{"line":20,"ch":17}},"message":"Expecting ')'","severity":"ERROR","className":"red_wavy_line"}]}}
    // 正常 {"text":"<outStream>0, 1\n</outStream>","exception":null,"errors":{"File.kt":[]}}
    // 运行时 异常 {"text":"","exception":{"message":"100","fullName":"java.lang.ArrayIndexOutOfBoundsException","stackTrace":[{"className":"TwoSum","methodName":"twoSum","fileName":"File.kt","lineNumber":12},{"className":"FileKt","methodName":"main","fileName":"File.kt","lineNumber":20},{"className":"FileKt","methodName":"main","fileName":"File.kt","lineNumber":-1}],"cause":null},"errors":{"File.kt":[]}}
    function result_handel(response) {
      if (response.exception != null) {
        return (
          JSON.stringify(response.exception, null, 2) + "\n" + response.text
        );
      }

      if (response.errors["File.kt"].length != 0) {
        return JSON.stringify(response.errors["File.kt"], null, 2);
      }

      return response.text.slice(11, -12);
    }
    fetch_with_timeout(
      ...
      .then(response => response.json())
      .then(response => (result_block.innerText = result_handel(response))) // rm <outStream> </outStream>
      .catch(
        error =>
  ...
  Array.from(document.querySelectorAll("pre code")).forEach(function(block) {
    ...
    // console.log(parent);
    var playgroundElement = document.createElement("pre");

    playgroundElement.className = "playpen";
    playgroundElement.appendChild(block);

    // parent.removeChild(block)
    parent.appendChild(playgroundElement);
  });
